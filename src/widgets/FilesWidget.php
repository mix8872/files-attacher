...
class FilesWidget extends \yii\base\Widget
{
    ...
    private function _setFiletypes($types, &$fileTypes, &$allowedFileTypes, &$allowedFileExtensions)
    {
        if (is_array($types)) {
            foreach ($types as $filetype) {
                ...
                $type[1] == "*" ? : $allowedFileExtensions[] = $this->_getExtensionByMime($filetype);
                $fileTypes = implode(',', $types);
            }
        } else {
            $type = explode('/', $types);
            $allowedFileTypes = $this->_getType($type[0]);
            $type[1] == "*" ? : $allowedFileExtensions[] = $this->_getExtensionByMime($types);
            $fileTypes = $types;
        }
    }
    ...
    private function _getExtensionByMime($mime)
    {
        $types = [
